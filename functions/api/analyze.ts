    ...
    try {
      if (supabaseUrl && supabaseKey) {
        ...
        const duelPayload = {
          ...
          tips: typeof tips.weaknesses_of_loser === 'string' ? tips.weaknesses_of_loser : JSON.stringify(tips.weaknesses_of_loser),
          reasons: typeof judgment.reasons_for_win === 'string' ? judgment.reasons_for_win : JSON.stringify(judgment.reasons_for_win),
          preview_a: photoA.startsWith('data:') ? photoA : `data:image/jpeg;base64,${photoA}`,
          preview_b: photoB.startsWith('data:') ? photoB : `data:image/jpeg;base64,${photoB}`,
          is_public: true,
